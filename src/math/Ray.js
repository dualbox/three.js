...
class Ray {
	...
	intersectCone( cone, target ) {
		...
		_E.subVectors( this.origin, cone.v );
		var AdE = cone.axis.dot( _E );
		var DdE = this.direction.dot( _E );
		var EdE = _E.dot( _E );
		...
		if ( Math.abs( c2 ) >= 0 ) {
			...
			if ( discr < 0 ) {
				...
				if ( t > 0 ) {
					...
					_E.subVectors( target, cone.v );
					dot = _E.dot( cone.axis );
				...
				if ( t2 > 0 && t2 < t ) {
					...
					_E.subVectors( _target2, cone.v );
					dot = _E.dot( cone.axis );
					if ( dot > cone.inf && dot < cone.sup ) {

						quantity ++;
						target.copy( _target2 );
	 ...
	 *
	 */
	intersectCylinder( cyl, target ) {

		_vtos.subVectors( this.origin, cyl.v );
		var vtos_dot_ax = _vtos.dot( cyl.axis );
		...
		_tmp2.set(
			_vtos.x - vtos_dot_ax * cyl.axis.x,
			_vtos.y - vtos_dot_ax * cyl.axis.y,
			_vtos.z - vtos_dot_ax * cyl.axis.z );

		var a = _tmp1.lengthSq();
		var b = 2 * _tmp1.dot( _tmp2 );
		var c = _tmp2.lengthSq() - cyl.radius * cyl.radius;
		...
		if ( delta < 0 ) {
			...
			_tmp.subVectors( target, cyl.v );
			var dot = _tmp.dot( cyl.axis );
			...
			_tmp.subVectors( _tmp2, cyl.v );
			dot = _tmp.dot( cyl.axis );
